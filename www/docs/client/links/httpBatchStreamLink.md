...
Receiving the stream relies on the `TextDecoder`, `WritableStream` and `TextDecoderStream` APIs, which is not available in React Native. If you still want to enable streaming, you need to polyfill those.
